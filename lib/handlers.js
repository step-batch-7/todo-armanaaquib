...
const addTodo = function (req, res) {
  ...
  fs.writeFileSync(TODO_LIST_STORE_PATH, todoListCollection.toJSON(), 'utf8');
...
const removeTodo = function (req, res) {
  const {todoListId} = querystring.parse(req.body);
...
const addTask = function (req, res) {
  const {todoListId, taskText} = querystring.parse(req.body);
...
const removeTask = function (req, res) {
  const {todoListId, taskId} = querystring.parse(req.body);
...
const updateTaskStatus = function (req, res) {
  const {todoListId, taskId, status} = querystring.parse(req.body);
